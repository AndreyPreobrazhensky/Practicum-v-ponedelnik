...
int main () {
    ...
    if (PAPI_event_name_to_code("PAPI_L1_TCM", &code) != PAPI_OK){
            std::cout << "convertion error" << std::endl;
            return 1;}
    if ((retval = PAPI_add_event(Eventset, PAPI_L1_TCM)) != PAPI_OK){
	    std::cout << "addition1 error " << retval << std::endl;
	    return 1;}
    if (PAPI_add_event(Eventset, PAPI_L2_TCM) != PAPI_OK){
	    std::cout << "addition2 error" << std::endl;
	    return 1;}
    if ((retval = PAPI_event_name_to_code("perf::PERF_COUNT_HW_CACHE_REFERENCES", &code)) != PAPI_OK){
	    std::cout << "convertion error" << retval << std::endl;
	    return 1;}
	...
	PAPI_stop(Eventset, values);
        std::cout << "1.papi reference: L1_TCM: " << values[0] << " L2_TCM: " << values[1] << " custom: " << values[2] << std::endl;
        PAPI_reset(Eventset);
       	PAPI_start(Eventset);	
	std::cout << "max weight ver:" << a.weightMAXver() << std::endl;
        PAPI_stop(Eventset, values);
	std::cout << "2.papi reference: L1_TCM: " << values[0] << " L2_TCM: " << values[1] << " custom: " << values[2] << std::endl;
