...
void game(int *setka, int *newsetka, int size, int rank){
	...
	if (rank == 0){
		MPI_Isend(setka, N, MPI_INT, size - 1, 0, MPI_COMM_WORLD, &req3);
		MPI_Irecv(setka + N * (N / size + 1), N, MPI_INT, size - 1, 0, MPI_COMM_WORLD, &req2);
	}
        else{
	        MPI_Isend(setka, N, MPI_INT, (rank - 1) % size, 0, MPI_COMM_WORLD, &req3);
	        MPI_Irecv(setka + N * (N / size + 1), N, MPI_INT, (rank - 1), 0, MPI_COMM_WORLD, &req2);
	}
	MPI_Isend(setka + N * (N / size + 1), N, MPI_INT, (rank + 1) % size, 0, MPI_COMM_WORLD, &req4);
